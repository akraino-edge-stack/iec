...
*)
    ...
    echo "#:~ > skip_net=1 ./$0 stop"
    echo "usage: $0 [start|stop] [net] [k8s_master] [k8s_slaves]"
    exit 1
    ;;
esac
...
case $cmd in
start)
    if [ -z "$stack_net" ] && [ -z "$skip_net" ]; then
        echo "Starting net"
...
#            --parameter volume_id=$master_vol \
    ...
    k8s_master_ip=$(openstack stack output show net k8s_master_ip -f value -c output_value)
    k8s_pod_net_cidr=$(openstack stack output show net k8s_pod_net_cidr -f value -c output_value)
    k8s_svc_net_cidr=$(openstack stack output show net k8s_svc_net_cidr -f value -c output_value)
    k8s_cluster_ip=$(openstack stack output show net k8s_cluster_ip -f value -c output_value)
    ...
    if [ -z "$stack_k8s_slaves" ] && [ -z "$skip_k8s_slaves" ]; then
        ...
        openstack stack create --wait \
            --parameter key_name="$key_name" \
            --parameter slaves_count="$slaves_count" \
            --parameter k8s_master_ip="$k8s_master_ip" \
            --parameter k8s_pod_net_cidr="$k8s_pod_net_cidr" \
            --parameter k8s_svc_net_cidr="$k8s_svc_net_cidr" \
            --parameter k8s_cluster_ip="$k8s_cluster_ip" \
...
stop)
    ...
    if [ -n "$stack_net" ] && [ -z "$skip_net" ]; then
        echo "Stopping net"
