...
deploy_k8s () {
  ...
  SETUP_CNI="source /root/.bashrc && cd iec/src/foundation/scripts && source setup-cni.sh"
  sshpass -p ${K8S_MASTERPW} ssh -o StrictHostKeyChecking=no ${HOST_USER}@${K8S_MASTER_IP} ${SETUP_CNI}
  SETUP_HELM="source /root/.bashrc && cd iec/src/foundation/scripts && source helm.sh"
